...
## 🧪 실험 중 (2026-07-14 추가 — 회사 확정 전, 데모 UI 토글로만 사용)

- [x] 손가락 1/2/3개(point/two/three)로 항목 번호 직접 선택 — `select_number` 이벤트
      (data.number). `gestures.number_select.enabled`는 기본 꺼짐, 데모 UI 상단 배지
      클릭으로 세션 중 켜고 끔(재시작 불필요). 정식 스펙 채택 여부는 №1과 함께
      회사 확인 필요 — 그 전엔 프로덕션 config에서 켜지 말 것.
