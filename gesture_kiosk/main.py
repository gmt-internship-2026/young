...
델파이(회사 UI)가 **이 파일을 직접 실행**한다 — bat 경유 없이:
    venv_win\\Scripts\\python.exe main.py            # 엔진만 — 이벤트가 stdout에 한 줄씩
    venv_win\\Scripts\\python.exe main.py --debug    # 카메라 창을 켠 채 시작(선택)
...
★환경 비종속(2026-08-03 — 시선추적과의 가상환경 통합 대비): 이 파일은 특정
venv에 묶이지 않는다 — 의존이 설치된 **아무 파이썬**으로 실행하면 된다
(자체 venv면 venv_win\\Scripts\\python.exe, 통합 환경이면 그 환경의 python.exe —
설치가이드.md "통합 환경 설치" 절). activate 불필요, 호출 위치(CWD)도 무관
