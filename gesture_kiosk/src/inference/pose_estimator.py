"""inference 모듈 — 사람 포즈(RTMPose)를 추론해 얼굴·어깨 키포인트를 얻는다.

역할(2026-07-28 손 모델 분리): 사람 검출·사용자 잠금(얼굴)·어깨 자(尺)를 담당한다.
손 모양·손 중심은 hand_tracker.py(MediaPipe)가 전담 — wholebody 손 21점의 유령 손·
원근 단축 실기 문제로 교체됐다 (경위는 hand_tracker.py 모듈 주석).
...
pose_engine=body(2026-07-28 기본 — 손 분리로 wholebody 필수 사유 소멸)면 17점,
wholebody면 앞 17개가 동일한 COCO-WholeBody 133 규격 (손 키포인트는 이제 미사용).
...
class PoseEstimator:
    ...
    def __init__(self, config):
        ...
        if engine == "wholebody":
            # 전신 133 — 07-28 손 모델 분리 후 손 키포인트는 미사용 (body로 충분, CPU만 더 씀)
