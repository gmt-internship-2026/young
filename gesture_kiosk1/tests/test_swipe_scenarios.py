...
2026-08-04 개편 스펙: 손 모양(주먹/한 손가락) × 방향 — 이벤트
temp_left/temp_right/temp_top/back/home/confirm (한 손가락 = 임시 계층 —
08-04 손바닥과 명칭 맞교환, 구 select 폐기. 손바닥의 left/right/top은
test_gesture_filter.py가 담당 — 이 파일의 _Sim은 기본 shape가 finger다.
상하(top/bottom) 중 아래 방향은 두 모양 다 정의 없음 = 무시).
...
class SwipeScenarioTest(unittest.TestCase):
    ...
    def test_01_finger_right(self):
        ...
        self._run(scenario, ["temp_right"])

    def test_02_finger_left(self):
        def scenario(sim):
            sim.hold(0.5)
            sim.move_by(-AMP_X, 0, 0.3)
        self._run(scenario, ["temp_left"])

    def test_03_finger_up_is_temp_top(self):
        # 2026-08-04 개편: 한 손가락+위 = temp_top(구 select — 손바닥과 명칭 맞교환).
        # 첫 hold 0.8초: 손 등장도 휴식 존 이력로 취급되므로(근거리 정정) 등장 직후
        # 유예(0.6초)가 지나야 위 방향이 열린다 — 실사용에선 탐색 후 temp_top이라 무영향
        def scenario(sim):
            sim.hold(0.8)
            sim.move_by(0, -AMP_Y, 0.3)
        self._run(scenario, ["temp_top"])
    ...
    def test_12_intentional_left_after_right_passes(self):
        ...
        self._run(scenario, ["temp_right", "temp_left"])
    ...
    # ----- 들어올리기 게이트 (temp_top·home 오발 방지) -----

    def test_16_arm_raise_then_down_fires_nothing(self):
        # 손을 내리고 있다가 들어올리는 동작이 temp_top으로 오발되면 안 된다 — 휴식 존
        # 이력 게이트 (2026-07-20). 이어지는 아래 쓸기도 07-29 개편으로 정의 없음
        def scenario(sim):
            sim.position = HANG             # 손 축 처진 상태(휴식 존 안)에서 시작
            sim.hold(0.5)
            sim.move_by(0, REST[1] - HANG[1], 0.4)   # 들어올리기 — temp_top 금지
            sim.hold(0.3)
            sim.move_by(0, AMP_Y, 0.3)               # 아래 쓸기 — 정의 없음(무시)
        self._run(scenario, [])

    def test_17_temp_top_after_settling_above_rest_zone(self):
        # 들어올린 뒤 유예(0.6초)를 넘겨 자세가 안정되면 위 스냅은 정상 temp_top
        def scenario(sim):
            sim.position = HANG
            sim.hold(0.5)
            sim.move_by(0, REST[1] - HANG[1], 0.4)   # 들어올리기 — 무시
            sim.hold(0.8)                            # 유예(0.6) 경과 — 손 든 채 안정
            sim.move_by(0, -AMP_Y, 0.3)              # 위 스냅 = 의도적 temp_top
        self._run(scenario, ["temp_top"])

    def test_18_raise_then_immediate_down_fires_nothing(self):
        # 들어올리기 직후 곧바로 아래로 내리는 동작 — 들어올리기(temp_top 금지)도
    ...
    def test_20_close_range_hand_appearance_then_down(self):
        ...
        # 등장→상승이 temp_top으로 오발되지 않아야 한다 (아래는 07-29부터 정의 없음)
        def scenario(sim):
            sim.drop(0, 0, 0.6)                  # 손 부재(화면 밖 — 추적점 없음)
            sim.position = (0.5, 0.40)           # 화면 하단(존 밖·어깨선 아래)에서 등장
            sim.move_by(0, -0.16, 0.25)          # 등장하며 올라옴 — temp_top 금지
            sim.hold(0.2)
            sim.move_by(0, AMP_Y, 0.3)           # 아래 — 정의 없음(무시)
        self._run(scenario, [])

    def test_21_close_range_appear_pause_then_temp_top(self):
        ...
        # 뒤의 위 플릭이 temp_top으로 나가야 한다 (쉼 없는 등장→상승은 test_20이
        # 여전히 차단을 검증)
        def scenario(sim):
            ...
            sim.move_by(0, -AMP_Y, 0.25)         # 위 플릭 = 의도적 temp_top
        self._run(scenario, ["temp_top"])
