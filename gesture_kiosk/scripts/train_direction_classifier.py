"""방향(left/right/up/down/none) 분류기 학습 — collect_direction_data.py가 모은 CSV로
로지스틱 회귀를 학습하고, models/weights/에 가중치를 내보낸다 (2026-07-24 도입).

scikit-learn은 이 학습 스크립트에서만 쓴다 — 추론 쪽(direction_classifier.py)에는
sklearn을 설치하지 않는다. 내보낸 .npz는 계수 행렬 + 절편 + 클래스 이름뿐이라 numpy
행렬곱 하나로 추론된다. train_hand_shape_classifier.py와 구조가 동일하다.

사용법 (gesture_kiosk 폴더에서, venv_win 활성화 후 — scikit-learn만 추가 설치 필요):
    venv_win\\Scripts\\pip.exe install scikit-learn
    venv_win\\Scripts\\python.exe scripts\\train_direction_classifier.py
...
DEFAULT_DATA_PATH = os.path.join(ROOT_DIR, "data", "direction", "tracks.csv")
DEFAULT_OUT_PATH = os.path.join(ROOT_DIR, "models", "weights", "direction_classifier.npz")
...
def main():
    parser = argparse.ArgumentParser(description="방향 분류기 학습(로지스틱 회귀)")
    parser.add_argument("--data", default=DEFAULT_DATA_PATH,
                         help="collect_direction_data.py가 만든 CSV")
    ...
    if len(x_train) < 150:
        print("[WARN] 샘플이 너무 적습니다(150건 미만) — 5-way 분류라 각 라벨당 최소 "
              "50~60건은 더 모으는 걸 권장(특히 none은 하위 패턴이 여러 개라 더 필요)")
    ...
    print("[다음 단계] configs/config.yaml과 configs/config_mac.yaml의 "
          "gestures.hand_move.classifier_weights_path 주석을 해제하고 "
          "'models/weights/direction_classifier.npz'로 설정하면 적용됩니다")
