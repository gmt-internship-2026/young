"""추적점 스무딩 — One Euro 필터 (2026-07-23, GMtech_project에서 이식).

포즈·손 키포인트는 정지 상태에서도 프레임마다 수 픽셀씩 떨린다(관측 잡음).
이 떨림이 쓸기 궤적에 섞이면 진행도가 출렁여 오탐의 원인이 된다(사용자 실기
리포트 — "아무것도 안 하는데 왼쪽으로 갔다 오른쪽으로 갔다 마음대로 확정됨").
